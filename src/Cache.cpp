...
static GTY((if_marked("tree2int_marked_p"),
            param_is(struct tree2int))) htab_t intCache;
...
static GTY((if_marked("tree2Type_marked_p"),
            param_is(struct tree2Type))) htab_t TypeCache;
...
static GTY((if_marked("tree2WeakVH_marked_p"),
            param_is(struct tree2WeakVH))) htab_t WeakVHCache;

#if (GCC_MAJOR >= 5)
#define ggc_alloc_tree2WeakVH()                                                \
  ((struct tree2WeakVH *)(ggc_internal_alloc(sizeof(struct tree2WeakVH)        \
                                                 MEM_STAT_INFO)))
#define ggc_alloc_tree2int()                                                   \
  ((struct tree2int *)(ggc_internal_alloc(sizeof(struct tree2int)              \
                                              MEM_STAT_INFO)))
#define ggc_alloc_tree2Type()                                                  \
  ((struct tree2Type *)(ggc_internal_alloc(sizeof(struct tree2Type)            \
                                               MEM_STAT_INFO)))
#endif
#if (GCC_MAJOR == 4)
